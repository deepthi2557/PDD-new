...
def generate_appium_excel_report(test_results, output_path=None):
    """
    Generates a multi-sheet, styled Excel Analysis Report for Appium E2E Test Execution.
    """
    if output_path is None:
        output_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "appium_test_analysis_report.xlsx")
    ...
    # Title Block
    ws_summary["A1"] = "Appium Mobile E2E Test Suite Analysis Report"
    ws_summary["A1"].font = title_font
    ws_summary["A2"] = "Target Web App: https://pdd-new.vercel.app/ | Multi-Tab Workflows & Button Feature Analysis"
    ...
    kpis = [
        ("TOTAL TEST CASES", total_tests, "A4"),
        ("PASSED TESTS", passed_tests, "C4"),
        ("FAILED TESTS", failed_tests, "E4"),
        ("PASS RATE", f"{pass_rate}%", "G4"),
        ("MULTI-TAB VERIFIED", multi_tab_count, "I4")
    ]

    for title, val, top_left in kpis:
    ...
    # Section Header
    ws_summary["A8"] = "Category Breakdown & Execution Metrics"
    ...
    wb.save(output_path)
    print(f"[APPIUM REPORT] Excel analysis successfully exported to: {output_path}")
    return output_path
